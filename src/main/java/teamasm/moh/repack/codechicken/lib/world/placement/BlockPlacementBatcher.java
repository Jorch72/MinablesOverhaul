package teamasm.moh.repack.codechicken.lib.world.placement;
 ...
 * Modified version of BlockPlacementBatcher from CCL, The CCL version was never finished and needs to be updated.
 * <p/>
 * CHANGES! (shouldn't be needed considering i wrote the original class but meh)
 * -Remove the entire relight chunk pipe, we only use it for underground stuff so its fine.
 */
public class BlockPlacementBatcher {
    ...
    public void setBlockState(BlockPos pos, IBlockState newState) {
        ...
        createBlockStorageIfNeeded(pos, null/*builder*/);
